...
@section('content')
    <div class="flex justify-center items-center py-14">
        <div
            class="w-full max-w-6xl p-8 bg-white border border-gray-200 rounded-lg shadow-lg dark:bg-gray-800 dark:border-gray-700">
            <form id="orderForm" action="{{ route('admin.update', ['id' => $data->id]) }}" method="post"
                ...
                <div class="grid gap-6 mb-6 md:grid-cols-1 px-8">
                    <div class="flex flex-col justify-start">
                        <div>
                            <label for="name_product" class="block mb-2 font-medium text-gray-900 dark:text-white">Nama
                                Barang</label>
                        </div>
                        <div>
                            <input type="text" id="name_product" name="name_product"
                                class="bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                required value="{{ old('name_product', $data->nama) }}" />
                        </div>
                    </div>
                    ...
                    <div>
                        <label for="color"
                            class="block mb-2 font-medium text-gray-900 dark:text-white text-2xl">Warna</label>
                        <div class="grid grid-cols-1 gap-2">
                            @php
                                // Gunakan nilai old atau data dari database
                                $selectedColors = old('colors', $data->warna);
                            @endphp
                            @foreach (['Hitam', 'Abu', 'Abu Tua', 'Hijau', 'Hijau Army', 'Coklat', 'Coklat Kopi', 'Cream', 'Olive', 'Khaki', 'Putih', 'Broken White'] as $color)
                                <div class="flex items-center justify-between border p-2 rounded-md">
                                    <span class="text-gray-900 dark:text-white">{{ $color }}</span>
                                    <input type="checkbox" id="{{ str_replace(' ', '_', $color) }}" name="colors[]"
                                        value="{{ $color }}" class="peer sr-only opacity-0"
                                        {{ in_array($color, $selectedColors) ? 'checked' : '' }} />
                                    <label for="{{ str_replace(' ', '_', $color) }}"
                    ...
                    <div>
                        <label for="size"
                            class="block mb-2 font-medium text-gray-900 dark:text-white text-2xl">Ukuran</label>
                        <div class="grid grid-cols-1 gap-2">
                            @php
                                // Gunakan nilai old atau data dari database
                                $selectedSizes = old('sizes', $data->size);
                            @endphp
                            @foreach (['S', 'M', 'L', 'XL', 'XXL', 'XXXL', 'VIXL'] as $size)
                                <div class="flex items-center justify-between border p-2 rounded-md">
                                    ...
                                    <input type="checkbox" id="{{ $size }}" name="sizes[]"
                                        value="{{ $size }}" class="peer sr-only opacity-0"
                ...
                <!-- Tombol plus dan minus untuk Quantity -->
                <div class="flex justify-center items-center mb-6 gap-2">
                    ...
                    <input type="number" id="quantity" name="quantity"
                        class="w-16 border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 p-2"
                        value="{{ old('quantity', $data->total) }}">
                ...
                <div class="flex justify-between">
                    <a href="{{ route('admin.home') }}"
                        class="bg-orange-500 hover:bg-orange-600 py-2.5 px-5 rounded-lg">Cancel</a>
                    <button type="submit"
                        class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg px-5 py-2.5 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">Kirim</button>
                </div>
            </form>
        </div>
    </div>
